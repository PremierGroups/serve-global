...
if (isset($_GET['blog-id'])) {
    ...
    if (empty($row['id'])) {
        header("location:blogs?msg=Please select the blog to edit!");
        exit(1);
    }
    $path = $row['coverImage'];
} else {
...
// if everything is ok, try to upload file
    ...
    $blog->updateBlog($id, $title, $coverImage, $description, $category);
...
<html>
    ...
    <title>Serve Global | Edit Blog </title>
        ...
        <div class="content-wrapper">
            <!-- Content Header (Page header) -->
                <section class="content-header">
                    <h1>
                        Dashboard
                        <small><span class="fa fa-edit"></span> Edit Blog</small>
                    </h1>
                    <ol class="breadcrumb">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading"><h4>Edit Blog</h4></div>
                            <form class="form-horizontal form-group" enctype="multipart/form-data" action="<?php $_PHP_SELF ?>" method="post" id="editNewsForm">
                                <div class="panel-body">
                                    <div class="form-group">
                                        <label for="inputTitle" class="col-sm-2 control-label">Title</label>
                                        <div class="col-sm-10">
                                            <input type="text" name="title" class="form-control" id="inputTitle"  value="<?php echo "$blogTitle"; ?>">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="inputSkill" class="col-sm-2 control-label">Category</label>
                                        <div class="col-sm-10">
                                            <select name="category" class="form-control" id="inputSkill" placeholder="Select Category...">
                                                <?php
                                                $cateObj = new Category();
                                                $categories = $cateObj->getAllCategories();
                                                while ($rowCat = mysqli_fetch_array($categories)) {
                                                    if ($categoryId == $rowCat['id']) {
                                                        echo "<option value='$rowCat[id]' selected='true'>" . ucfirst($rowCat['name']) . "</option>";
                                                    } else {
                                                        echo "<option value='$rowCat[id]'>" . ucfirst($rowCat['name']) . "</option>";
                                                    }
                                                }
                                                ?>
                                            </select>
...
if (file_exists("../images/" . $path)) {
    ?>
                                                <a href="../images/<?php echo "$path"; ?>" data-toggle="lightbox" data-title="<?php echo $blogTitle; ?>" data-gallery="gallery">
                                                    <p  class="img-fluid mb-2 label label-primary"> <span class="fa fa-eye"></span> View Image</p>
                                                </a>
    <?php
} else {
    ?>
                                                <p  class="img-fluid mb-2 label label-warning"> <span class="fa fa-exclamation-triangle"></span> Image not Found</p>
                                                <?php
                                            }
                                            ?>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <textarea class="form-control textarea" id="inputDescription" name="description" rows="5"
                                                      style="width: 100%; height: 300px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo $description; ?>
                                            </textarea>
                                        </div>
                                ...
                                <div class="panel-footer">
                                    <div class="form-group">
                                        <div class="col-sm-offset-2 col-sm-10">
                                            <button type="submit" class="btn btn-primary" name="updateNewsBtn"><span class="fa fa-save"></span> Update</button>
                                            <a href="blogs" class="btn btn-default"><span class="fa fa-backward"></span> Cancel</a>
                                        </div>
                                    </div>
                                </div>
                            </form>
        ...
        $(document).ready(function () {
            ...
            $('#editNewsForm').bootstrapValidator({
                ...
                fields: {
                    ...
                    description: {
                        message: 'Blog Description is not valid',
                        validators: {
                            notEmpty: {
                            ...
                            stringLength: {
                                min: 20,
                                max: 10000,
                                message: 'Description must be more than 20 and less than 700 characters long'
                            }
                        }
                    }
